...
def run(sequences: Dict, shape_sequence: str, output_dir: str, resize: int,
    ...
    seq_dirs, frame_patterns, crop_patterns, mask_patterns = {}, {}, {}, {}
    deca_dirs, segmentation_dirs, normals_dirs = {}, {}, {}
    
    for seq_name in sequences:
        seq_dir = output_dir / str(seq_name)
        seq_dirs[seq_name] = seq_dir
        frame_patterns[seq_name] = (seq_dir / "image" / "%d.png").resolve()
        crop_patterns[seq_name] = (seq_dir / "crops" / "%d.png").resolve()
        mask_patterns[seq_name] = (seq_dir / "mask" / "%d.png").resolve()
        deca_dirs[seq_name] = (seq_dir / "deca").resolve()
        segmentation_dirs[seq_name] = (seq_dir / "semantic").resolve()
        normals_dirs[seq_name] = (seq_dir / "normals").resolve()

    start_time = time.time()

    ############################################################
    if "extract" in steps:
        FRAME_RATE = 30   #修改提取帧率
    ...
    if "crop" in steps:
        ...
        for seq_name, seq in sequences.items():
            ...
            cmd = f"python compute/detect_crops.py --batch_size 8 --resize {resize} --scale {crop_scale} --input '{frame_patterns[seq_name].parent}' --output '{crop_patterns[seq_name].parent}'"
            cmd += f" --mode {seq.crop_mode if 'crop_mode' in seq else crop_mode}"
            cmd += f" --face_selection_strategy {seq.face_selection_strategy}"
            if "fixed_box" in seq:
                cmd += f" --fixed_box {' '.join(map(str, seq.fixed_box))}"
            cmds.append(cmd)
